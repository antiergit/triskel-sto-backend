...
class Database implements DBInterface {
  ...
  constructor() {
    ...
    this.connectionWrite = `mysql://${config.DB.DB_USER}:${config.DB.DB_PASSWORD}@${config.DB.DB_HOST_WRITE}/${config.DB.OLD_DB_NAME}`;

    this.connectionRead = `mysql://${config.DB.DB_USER}:${config.DB.DB_PASSWORD}@${config.DB.DB_HOST_READ}/${config.DB.OLD_DB_NAME}`;
